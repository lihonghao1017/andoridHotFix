...
public class EasyTouchView extends View {
    ...
    private void initEasyTouchViewEvent() {
        ...
        mWMParams = wmParams;
        wmParams.type = LayoutParams.TYPE_APPLICATION_OVERLAY;; // 这里的2002表示系统级窗口，你也可以试试2003。
    ...
    private OnTouchListener mTouchListener = new OnTouchListener() {
        ...
        private void motionActionUpEvent(float x, float y) {
            ...
            if (mOldOffsetX == newOffsetX && mOldOffsetY == newOffsetY) {
                ...
                mPopuWin.setBackgroundDrawable(new ColorDrawable(0x00ffffff));
...
//                mPopuWin.setContentView(mSettingTable);
                ...
                mPopuWin.setFocusable(true);
                mPopuWin.setWindowLayoutType(WindowManager.LayoutParams.FLAG_FULLSCREEN        );
//                mPopuWin.update();

                if (Build.VERSION.SDK_INT >= 24) {
                    int[] point = new int[2];
                    mTouchView.getLocationInWindow(point);
                    mPopuWin.showAtLocation(mTouchView.getRootView(), Gravity.NO_GRAVITY, 0, point[1] + mTouchView.getHeight()+200);
                } else {
                    mPopuWin.showAsDropDown(mTouchView);
                }
